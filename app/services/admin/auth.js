...
async function login(locale, authHeader, paramsValues) {
  try {
    ...
    } else if (user.usersGroups[0].gym && user.usersGroups[0].gym.isDelete) {
      return errorMessage(i18n.__('gyms.isDelete'), 404);
    } else if (
      user.usersGroups[0].groupId === 10 ||
      user.usersGroups[0].groupId === 11
    ) {
    ...
    // Builds token
    var token = await auth.sign(payload, config.JWT_KEY_ADMIN);
